...
mixin IRSManager {
  ...
  Future<void> syncResource(String key);
}
...
class RSManager implements IRSManager {
  ...
  @override
  void languageChange(int language) => _languageController.add(language);
...
class Resource {
  ...
  Box<ResourceEntity> get box => _box;
  ...
  init({Box<ResourceEntity>? box}) async {
    final service = data.ConfigService(Dio());
    if (box != null) {
      _box = box;
    } else {
      _box = await Hive.openBox<ResourceEntity>(
        'resources',
        bytes: Uint8List(0),
      );
    }
    ...
    manager = RSProxy(RSManager(_box, service));

    manager.languageChange(1);
